...
**Allocate.** This is the stage everything else exists to serve. `exocortex-next` reads your current state — open loops, how stale each scope's `STATE.md` has gone, deadlines, the energy you reported at your last check-in, and what's captured but still unshipped — and scores a handful of candidate moves against a named set of factors: staleness, deadline pressure, energy match, shippable-but-unfinished, surfaced priority, queue pressure. It returns the best one to three, each with the factor breakdown that produced it, so you can see *why* a move won and overrule it when your gut disagrees. Run `exocortex-next --why` and the math is right there. The scoring is a transparent weighted sum on purpose — it lives in one place so a learned policy can drop in behind the same interface without touching anything downstream, and every suggestion is logged next to what you actually did about it. That log is the training set the policy will learn from.

**Compose.** When you launch a session, the context is built before the model gets the first token. Your location in the tree sets the scope; ExoCortex loads that scope's contract files plus the same files from every ancestor up to root, so a project inherits the domain's stance and the root's rules without repeating them. The contracts load in lifecycle order — static identity (`README`, `AGENT`), the one live-state file (`STATE.md`, which carries staleness flags), accumulated judgment (`MEMORY`, `DECISION RULES`), then pointers — and the brief rides on top. It's lazy: an empty contract is preload noise, so absent files are skipped. The whole assembly is inspectable — `exocortex-doctor` prints the exact file list and the total character count it loaded — and the same composition runs under `claude`, `codex`, and `gemini` through a thin adapter, so the harness underneath stays swappable.

**Capture.** Nothing is left to memory. The wrapper tees the live session to a transcript as you work, and a `Stop` hook covers any session you launched without the wrapper, so the record is complete either way. A shared processed-ids registry deduplicates the two paths, so the same session is never summarized twice even when the hook and the wrapper both fire. Headless runs and observer subprocesses are filtered out, so what gets recorded is real work and not the machinery watching it. The output of this stage is a plain transcript plus a manifest — durable, greppable, and yours.

**Compound.** This is where a session turns into something the next session can use, and it's deliberately the most cautious stage. A model-backed worker reads each transcript and *stages* candidate promotions — a durable memory, a new decision rule, a `STATE.md` update, a wiki claim, a skill — into a review queue; it writes nothing durable on its own and surfaces its own failures instead of swallowing them, so a bad summarization run is visible rather than silently poisoning your memory. You clear the queue with `exocortex-review` (accept, reject, defer, or triage the backlog in one pass), and that review is the *only* place a candidate becomes a durable note. Every stateful change goes through the Logbook — append-only and reversible — so you can always see what changed and walk it back. In parallel, sessions roll up into weekly and monthly syntheses, so the memory stays legible as it accumulates instead of becoming a pile.

**Externalize.** The loop only pays off when something leaves your head, so finishing is a first-class stage, not an afterthought. The ship tracker follows a thread through captured → shaped → shipped and nudges you when something has sat captured too long going nowhere. `exocortex-ship shape` hands a thread straight to a writing skill and emits the prompt to start, collapsing "I should write this up" into one command. And because the allocator reads the tracker, an unfinished-but-shippable thread competes for your attention like everything else — so on the right day, *finish the essay* is literally the top-scored next move.
