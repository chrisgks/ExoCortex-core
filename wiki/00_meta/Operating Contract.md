...
## Knowledge-Steward Jobs

These four jobs define the knowledge-steward's active responsibilities beyond reactive wiki maintenance.

---

### Job 1 — Wiki Init

**Trigger:** new project scaffolded (via `init.py project`), or explicitly invoked

**Steps:**
1. Read the project's `README.md` and `AGENT.md`
2. Generate `wiki/00_meta/Scope.md` — topical categories, what belongs here vs. parent wiki
3. Update `wiki/index.md` with initial structure reflecting the project's domain
4. Update root `wiki-map.md` with this project's topics and status

---

### Job 2 — Session Synthesis

**Trigger:** post-session (invoked manually or when a synthesis threshold is reached)

**Steps:**
1. Query claude-mem `localhost:37777/api/search` for session summaries and high-signal observations (type: decision, discovery) from the relevant project
2. Extract what is wiki-worthy (durable knowledge vs. ephemeral action)
3. Create candidates carrying full signal: why_promoted, source (claude-mem observation ID), confidence, recurrence, priority
4. Route candidates through the Promotion Router (Job 3)

---

### Job 3 — Promotion Router

Four signals determine routing tier:

| Condition | Routing |
|---|---|
| Contradicts existing wiki content | Surface now — always |
| Matches current STATE.md focus + medium+ confidence | Surface now |
| Confirmed pattern (3+ occurrences) + low risk | Auto-apply |
| Everything else | Queue in inbox |

**Auto-apply:** write directly to the target file (wiki page, MEMORY.md, DECISION RULES.md, WORKFLOWS.md).
**Queue:** append candidate to the relevant `journal/inbox/pending-*.md` with full signal fields.
**Surface now:** append candidate to `journal/inbox/pending-*.md` with `priority: surface-now` and note it to the user at session start.

---

### Job 4 — Wiki-Map Maintenance

**Trigger:** after any wiki write (page created, updated, or deleted)

**Steps:**
1. Update `wiki-map.md` — add new topics when concept or analysis pages are created, update summaries when content changes, remove topics when pages are deleted
2. Keep entries accurate: one topic per meaningful theme, one wiki path per entry
