...
def ensure_instance_contracts(root: Path, force: bool) -> WriteSummary:
    """Write the starter root + ``system/`` contracts for a fresh instance.

    Lazy by design: only *non-empty* contracts are
    seeded. We deliberately do NOT create empty ``MEMORY.md`` / ``STATE.md`` /
    ``WORKFLOWS.md`` stubs — those exist only once they have real content and
    accrue by promotion. The files written here are the static-identity layer
    (rarely-changing) plus the minimal system contracts needed for the engine
    to run against the instance.
    """
    summary = WriteSummary(created=[], preserved=[])
    contracts = {
        "README.md": """
            ...
            This is an ExoCortex *instance* — your own data directory. The
            ExoCortex *engine* (the installed package) reads and writes here;
            this directory holds your journal, raw sources, wiki, and domains.

            Start a wrapped harness from this directory (or a sub-folder), or
            point the engine at it with `--root` / `$EXOCORTEX_HOME`.
            """,
        "AGENT.md": """
            # Root Agent Contract

            Default agent at the instance root: `chief-of-staff`.

            Operate from the most specific relevant scope. Infer from local and
            parent context first; ask only when ambiguity changes the outcome.
            Surface durable learnings as candidates for memory, skills, or
            decision rules rather than writing them silently.
            """,
        "system/README.md": """
            # System

            Instance-wide operating contracts for the ExoCortex outer loop.
            Files here govern how the engine composes context, captures work,
            and decides what compounds. Keep them sparse; grow by promotion.
            """,
        "system/AGENT.md": """
            # System Agent Contract

            The system scope holds cross-cutting operating rules. Treat these
            as authoritative for engine behavior. Prefer editing an existing
            rule over adding a redundant one.
            """,
    }
    for relative_path, content in contracts.items():
        ensure_text(root / relative_path, content, force, summary)
    # Structural directories an instance always has, even before they hold
    # content. The engine resolves an instance by these markers.
    for directory in ("agents", "domains", "system"):
        (root / directory).mkdir(parents=True, exist_ok=True)
    return summary


def init_instance(target: Path, force: bool) -> WriteSummary:
    """Scaffold a fresh ExoCortex *instance* at ``target`` from a clean slate.

    Produces a working instance the engine can immediately run against:
    starter root + ``system/`` contracts (lazy — non-empty only), the
    clean-slate ``journal/`` / ``raw/`` / ``wiki/`` scaffolds, and an empty
    review inbox. Seeds no personal content.
    """
    target = target.resolve()
    target.mkdir(parents=True, exist_ok=True)
    contracts = ensure_instance_contracts(target, force)
    runtime = ensure_repo_runtime(target, force)
    return WriteSummary(
        created=contracts.created + runtime.created,
        preserved=contracts.preserved + runtime.preserved,
    )
...
def init_project(root: Path, domain: str, name: str, force: bool) -> WriteSummary:
    ...
    title = titleize(name)
    # Lazy contracts: seed only the static-identity files
    # (README, AGENT) plus the managed-wiki scaffold. We deliberately do NOT
    # seed empty STATE.md / MEMORY.md / WORKFLOWS.md stubs — live state and
    # accumulated judgment appear only once there is real content to hold, and
    # WORKFLOWS.md is retired (sequences graduate to a skill or to MEMORY.md).
    project_summary = scaffold_from_templates(
        ...
        templates=[
            ...
            ("project-context/wiki/index.template.md", f"domains/{domain}/projects/{name}/wiki/index.md"),
            ("project-context/wiki/log.template.md", f"domains/{domain}/projects/{name}/wiki/log.md"),
            ("project-context/wiki/00_meta/Scope.template.md", f"domains/{domain}/projects/{name}/wiki/00_meta/Scope.md"),
            ("project-context/wiki/00_meta/Operating Contract.template.md", f"domains/{domain}/projects/{name}/wiki/00_meta/Operating Contract.md"),
            ("project-context/wiki/01_overviews/README.template.md", f"domains/{domain}/projects/{name}/wiki/01_overviews/README.md"),
            ("project-context/wiki/03_concepts/README.template.md", f"domains/{domain}/projects/{name}/wiki/03_concepts/README.md"),
            ("project-context/wiki/04_analyses/README.template.md", f"domains/{domain}/projects/{name}/wiki/04_analyses/README.md"),
            ("project-context/wiki/sources/README.template.md", f"domains/{domain}/projects/{name}/wiki/05_sources/README.md"),
        ],
        values={
            "DOMAIN_NAME": domain,
...
def parse_args() -> argparse.Namespace:
    ...
    instance_parser = subparsers.add_parser(
        "instance",
        help="Scaffold a fresh, clean-slate ExoCortex instance (data dir) at the given path.",
    )
    instance_parser.add_argument("dir", help="Target directory for the new instance. Created if absent.")
...
def main() -> int:
    ...
    if command == "instance":
        ...
        summary = init_instance(target, args.force)
        print_summary(f"ExoCortex instance scaffolded at {target.resolve()}", summary)
        print("Next steps:")
        print(f"  1. cd {target.resolve()}")
        print("  2. Run exocortex-health to confirm the engine can read the instance.")
        print("  3. Start a wrapped harness, or set $EXOCORTEX_HOME to this path.")
        return 0
